...
def enrc_predict(
    ...
    for i, centers_i in enumerate(centers):
        if use_P:
            current_P = P[i]
            weighted_squared_diff = squared_euclidean_distance(
                z_rot[:, current_P], centers_i[:, current_P]
            )
        else:
            current_subspace_betas = subspace_betas[i, :]
            weighted_squared_diff = squared_euclidean_distance(
                z_rot, centers_i, weights=current_subspace_betas
            )
        labels_sub = (
            weighted_squared_diff.argmin(1).detach().cpu().numpy().astype(np.int32)
        )
