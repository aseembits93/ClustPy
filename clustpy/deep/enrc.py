...
def _get_P(
    ...
    shared_space_idx = [
        i for i, centers_i in enumerate(centers) if centers_i.shape[0] == 1
    ]
    if shared_space_idx:
...
def _rotate(z: torch.Tensor, V: torch.Tensor) -> torch.Tensor:
    """
    Rotate the embedded data point z using the orthogonal rotation matrix V.
...
def enrc_encode_decode_batchwise_with_loss(
    ...
    V = V.to(device)  # Move V to device once
    with torch.no_grad():
        for batch in dataloader:
            batch_data = batch[1].to(device)
            z = model.encode(batch_data)
            z_rot = _rotate(z, V)
            embeddings.append(z_rot)
            z_rot_back = _rotate_back(z_rot, V)
            reconstruction = model.decode(z_rot_back)
    ...
    # Collect the results and move them to CPU only once
