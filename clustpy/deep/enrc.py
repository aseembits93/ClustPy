...
def _determine_sgd_init_costs(
    ...
    with torch.no_grad():
        for batch in dataloader:
            batch = batch[1].to(device, non_blocking=True)
            subspace_loss, z_rot, batch_rot_back, _ = enrc(batch)
            total_cost += subspace_loss.item()

            if return_rot:
                rotated_data.append(z_rot.detach().cpu())

            ssl_loss = ssl_loss_fn(batch_rot_back, batch).item()
            total_cost += ssl_loss

        averaged_cost = total_cost / len(dataloader)

    if return_rot:
        rotated_data = torch.cat(rotated_data).numpy()
        return averaged_cost, rotated_data
    else:
