...
def enrc_predict(
    ...
    """
    z_rot = _rotate(z, V)
    P = _get_P(betas=subspace_betas.detach(), centers=centers) if use_P else None

    labels = []
    for i, centers_i in enumerate(centers):
        projected_z_rot = z_rot[:, P[i]] if use_P else z_rot
        weights = None if use_P else subspace_betas[i, :]
        weighted_squared_diff = squared_euclidean_distance(
            projected_z_rot, centers_i, weights=weights
        )
        labels_sub = weighted_squared_diff.argmin(1).cpu().numpy().astype(np.int32)
