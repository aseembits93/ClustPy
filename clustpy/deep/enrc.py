...
class _ENRC_Module(torch.nn.Module):
    ...
    def __init__(
        ...
        P: list,
        V: np.ndarray,
        beta_init_value: float = 0.9,
        clustering_loss_weight: float = 1.0,
        ssl_loss_weight: float = 1.0,
        center_lr: float = 0.5,
        rotate_centers: bool = False,
        beta_weights: np.ndarray = None,
        ...
        if beta_weights is None:
            beta_weights = beta_weights_init(
                self.P, n_dims=centers[0].shape[1], high_value=beta_init_value
            )
        else:
            beta_weights = torch.tensor(beta_weights).float()
        self.beta_weights = torch.nn.Parameter(beta_weights, requires_grad=True)
        self.V = torch.nn.Parameter(
            torch.tensor(V, dtype=torch.float), requires_grad=True
        )
        ...
        self.centers = [
            torch.tensor(centers_sub, dtype=torch.float32) for centers_sub in centers
        ]
        ...
        for centers_i in self.centers:
            self.lonely_centers_count.append(
                np.zeros((centers_i.shape[0], 1)).astype(int)
            )
    ...
    def update_center(
        self, data: torch.Tensor, one_hot_mask: torch.Tensor, subspace_id: int
    ) -> None:
        """
        ...
        if self.centers[subspace_id].shape[0] == 1:
            ...
            self.centers[subspace_id] = (
                self.centers[subspace_id] * 0.5 + data.mean(0).unsqueeze(0) * 0.5
            )
        else:
            ...
            self.mask_sum[subspace_id][nonzero_mask] = (
                self.center_lr * mask_sum[nonzero_mask]
                + (1 - self.center_lr) * self.mask_sum[subspace_id][nonzero_mask]
            )

            per_center_lr = 1.0 / (1 + self.mask_sum[subspace_id][nonzero_mask])
            self.centers[subspace_id] = (1.0 - per_center_lr) * self.centers[
                subspace_id
            ][nonzero_mask] + per_center_lr * batch_cluster_sums[
                nonzero_mask
            ] / mask_sum[nonzero_mask]
    ...
    def forward(
        self, z: torch.Tensor, assignment_matrix_dict: dict = None
        ...
        for i, centers_i in enumerate(self.centers):
            weighted_squared_diff = squared_euclidean_distance(
                z_rot, centers_i.detach(), weights=subspace_betas[i, :]
            )
    ...
    def predict_batchwise(
        ...
        predicted_labels : np.ndarray
            n x c matrix, where n is the number of data points in z and c is the number of clusterings.
        """
        predicted_labels = enrc_predict_batchwise(
            V=self.V,
            centers=self.centers,
            model=model,
    ...
    def recluster(
        ...
        centers_reclustered, P, new_V, beta_weights = enrc_init(
            data=embedded_rot,
        ...
        self.P = P
        self.m = [len(P_i) for P_i in self.P]
        self.beta_weights = torch.nn.Parameter(
            torch.from_numpy(beta_weights).float(), requires_grad=True
        )
        self.V = torch.nn.Parameter(
            torch.tensor(new_V, dtype=torch.float), requires_grad=True
        )
        self.centers = [
            torch.tensor(centers_sub, dtype=torch.float32)
            for centers_sub in centers_reclustered
        ]
    ...
    def fit(
        ...
        if fix_rec_error:
            if debug:
                print("Calculate initial reconstruction error")
            _, _, init_ssl_loss = enrc_encode_decode_batchwise_with_loss(
                V=self.V,
                centers=self.centers,
                model=model,
            ...
            init_ssl_loss += 1e-8
            if debug:
                print("Initial reconstruction error is ", init_ssl_loss)
        i = 0
        labels_old = None
        ...
        for _ in tbar:
            ...
            for batch in trainloader:
                ...
                if self.augmentation_invariance:
                    ...
                    # reuse assignments
                    subspace_loss_aug, _, z_rot_back_aug, _ = self(
                        z_aug, assignment_matrix_dict=assignment_matrix_dict
                    )
                ...
                if fix_rec_error:
                    rec_weight = (
                        ssl_loss.item() / init_ssl_loss
                        + subspace_loss.item() / ssl_loss.item()
                    )
                    if rec_weight < 1:
                        rec_weight = 1.0
                    ssl_loss *= rec_weight
                ...
                summed_loss = (
                    self.clustering_loss_weight * subspace_loss
                    + self.ssl_loss_weight * ssl_loss
                )
            ...
            if tolerance_threshold is not None and tolerance_threshold > 0:
                ...
                labels_new = self.predict_batchwise(
                    model=model, dataloader=evalloader, device=device, use_P=True
                )
                if _are_labels_equal(
...
def _get_P(
    betas: torch.Tensor, centers: list, shared_space_variation: float = 0.05
) -> float:
    """
    ...
    P : list
        list containing indices for projections for each clustering
    """
    # Check if a shared space with a single cluster center exists
    shared_space_idx = next(
        (i for i, centers_i in enumerate(centers) if centers_i.shape[0] == 1), None
    )
    if shared_space_idx is not None:
        equal_threshold = 1.0 / betas.shape[0] - shared_space_variation
        # Increase Weight of shared space dimensions that are close to the uniform distribution
        mask = betas[shared_space_idx] > equal_threshold
        betas[shared_space_idx] += mask.float()
...
def enrc_predict(
    ...
    for i, centers_i in enumerate(centers):
        if use_P:
            weighted_squared_diff = squared_euclidean_distance(
                z_rot[:, P[i]], centers_i[:, P[i]]
            )
        else:
            weighted_squared_diff = squared_euclidean_distance(
                z_rot, centers_i, weights=subspace_betas[i, :]
            )
...
def enrc_predict_batchwise(
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            z = model.encode(batch_data)
            pred_i = enrc_predict(
                z=z, V=V, centers=centers, subspace_betas=subspace_betas, use_P=use_P
            )
            predictions.append(pred_i)
    return np.concatenate(predictions)


def enrc_encode_decode_batchwise_with_loss(
    V: torch.Tensor,
    centers: list,
    model: torch.nn.Module,
    dataloader: torch.utils.data.DataLoader,
    device: torch.device = torch.device("cpu"),
    ssl_loss_fn: torch.nn.modules.loss._Loss = None,
) -> np.ndarray:
    """
...
def optimal_beta(
    kmeans_loss: torch.Tensor, other_losses_mean_sum: torch.Tensor
) -> torch.Tensor:
    """
...
def calculate_optimal_beta_weights_special_case(
    data: torch.Tensor, centers: list, V: torch.Tensor, batch_size: int = 32
) -> torch.Tensor:
    """
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            for i, centers_i in enumerate(centers):
                ...
                weighted_squared_diff = squared_euclidean_distance(
                    z_rot.unsqueeze(1), centers_i.unsqueeze(1)
                )
                assignments = weighted_squared_diff.detach().sum(2).argmin(1)
                if len(set(assignments.tolist())) > 1:
                    one_hot_mask = int_to_one_hot(assignments, centers_i.shape[0])
                    weighted_squared_diff_masked = (
                        weighted_squared_diff * one_hot_mask.unsqueeze(2)
                    )
                else:
...
def calculate_beta_weight(
    data: torch.Tensor,
    centers: list,
    V: torch.Tensor,
    P: list,
    high_beta_value: float = 0.9,
) -> torch.Tensor:
    """
    The beta weights have a closed form solution if we have two subspaces, so the optimal values given the data, centers and V can be computed.
    See supplement of Lukas Miklautz, Lena G. M. Bauer, Dominik Mautz, Sebastian Tschiatschek, Christian Boehm, Claudia Plant: Details (Don't) Matter: Isolating Cluster Information in Deep Embedded Spaces. IJCAI 2021: 2826-2832
    ...
    if n_clusterings == 2:
        beta_weights = calculate_optimal_beta_weights_special_case(
            data=data, centers=centers, V=V
        )
    elif n_clusterings > 2:
        beta_weights = beta_weights_init(
            P=P, n_dims=data.shape[1], high_value=high_beta_value
        )
    else:
        raise ValueError(
            f"Number of clusterings is {n_clusterings}, but should be >= 2"
        )
    return beta_weights
...
def nrkmeans_init(
    ...
    for i in range(rounds):
        ...
        nrkmeans.fit(X=data)
        centers_i, P_i, V_i, scatter_matrices_i = (
            nrkmeans.cluster_centers,
            nrkmeans.P,
            nrkmeans.V,
            nrkmeans.scatter_matrices_,
        )
        if len(P_i) != len(n_clusters):
            ...
            cost = _get_total_cost_function(
                V=V_i, P=P_i, scatter_matrices=scatter_matrices_i
            )
            if lowest > cost:
                best = [
                    centers_i,
            ...
            if debug:
                print(
                    f"Round {i}: Found solution with: {cost} (current best: {lowest})"
                )
    ...
    centers = [np.matmul(centers_sub, V) for centers_sub in centers]
    beta_weights = calculate_beta_weight(
...
def sgd_init(
    ...
    for round_i in range(rounds):
        ...
        init_centers, P_init, V_init, _ = random_nrkmeans_init(
            data=data,
            n_clusters=n_clusters,
            rounds=10,
            input_centers=input_centers,
        ...
        enrc_module = _ENRC_Module(
            init_centers, P_init, V_init, beta_init_value=1.0 / len(P_init)
        ).to_device(device)
        ...
        param_dict = [
            dict({"params": [enrc_module.V]}, **optimizer_params),
        ...
        cost = _determine_sgd_init_costs(
            enrc=enrc_module,
            dataloader=dataloader,
            ssl_loss_fn=torch.nn.MSELoss(),
            device=device,
        ...
        if lowest > cost:
            best = [
                ...
                enrc_module.P,
                enrc_module.V,
                enrc_module.beta_weights,
            ]
            lowest = cost
        if debug:
            print(
                f"Round {round_i}: Found solution with: {cost} (current best: {lowest})"
            )

    centers, P, V, beta_weights = best
    beta_weights = calculate_beta_weight(
        data=torch.from_numpy(data).float(), centers=centers, V=V, P=P
    )
...
def acedec_init(
    ...
    for round_i in range(acedec_rounds):
        ...
        cost, z_rot = _determine_sgd_init_costs(
            enrc=enrc_module,
            dataloader=dataloader,
            ssl_loss_fn=torch.nn.MSELoss(),
            device=device,
        ...
        enrc_module.centers = [
            torch.tensor(centers_sub, dtype=torch.float32)
            for centers_sub in enrc_rotated_centers
        ]
...
def enrc_init(
    ...
    if init == "nrkmeans" or init == "subkmeans":
        centers, P, V, beta_weights = nrkmeans_init(
            data=data,
            n_clusters=n_clusters,
            rounds=rounds,
            input_centers=input_centers,
        ...
        centers, P, V, beta_weights = random_nrkmeans_init(
            data=data,
            n_clusters=n_clusters,
            rounds=rounds,
            input_centers=input_centers,
        ...
        centers, P, V, beta_weights = sgd_init(
            data=data,
            n_clusters=n_clusters,
            optimizer_params=optimizer_params,
            rounds=rounds,
        ...
        centers, P, V, beta_weights = acedec_init(
            data=data,
            n_clusters=n_clusters,
            optimizer_params=optimizer_params,
            rounds=rounds,
        ...
        if data.shape[0] > 100000 or data.shape[1] > 1000:
            ...
            init = "nrkmeans"
        centers, P, V, beta_weights = enrc_init(
            data=data,
            n_clusters=n_clusters,
            device=device,
            init=init,
...
def _calculate_rotated_embeddings_and_distances_for_n_samples(
    enrc: _ENRC_Module,
    model: torch.nn.Module,
    dataloader: torch.utils.data.DataLoader,
    n_samples: int,
    ...
    for batch in dataloader:
        ...
        if calc_distances:
            ...
            idx_other_centers = [
                i for i in range(subspace_centers.shape[0]) if i != center_id
            ]
            weighted_squared_diff = squared_euclidean_distance(
                z_rot, subspace_centers[idx_other_centers], weights=subspace_betas
            )
...
def _split_most_expensive_cluster(
    distances: torch.Tensor, z: torch.Tensor
) -> torch.Tensor:
    """
...
def reinit_centers(
    ...
    with torch.no_grad():
        ...
        for center_id, count_i in enumerate(
            enrc.lonely_centers_count[subspace_id].flatten()
        ):
            if count_i > enrc.reinit_threshold:
                ...
                if split == "cost":
                    embedding_rot, dists = (
                        _calculate_rotated_embeddings_and_distances_for_n_samples(
                            enrc,
                            model,
                            dataloader,
                            n_samples,
                    ...
                    new_center = _split_most_expensive_cluster(
                        distances=dists, z=embedding_rot
                    )
                elif split == "random":
                    embedding_rot, _ = (
                        _calculate_rotated_embeddings_and_distances_for_n_samples(
                    ...
                    raise NotImplementedError(
                        f"split={split} is not implemented. Has to be 'cost' or 'random'."
                    )
                enrc.centers[subspace_id][center_id, :] = new_center.to(device)

                embeddingloader = torch.utils.data.DataLoader(
                    embedding_rot,
                    batch_size=dataloader.batch_size,
                    shuffle=False,
                    drop_last=False,
...
def _are_labels_equal(
    labels_new: np.ndarray, labels_old: np.ndarray, threshold: float = None
) -> bool:
    """
    ...
    if (
        labels_new is None
        or labels_old is None
        or labels_new.shape[1] != labels_old.shape[1]
    ):
        return False
    ...
    return all(
        [
            normalized_mutual_info_score(
                labels_new[:, i], labels_old[:, i], average_method="arithmetic"
            )
            >= v
...
def _enrc(
    ...
    # Setup dataloaders
    trainloader, testloader, batch_size = get_train_and_test_dataloader(
        X, batch_size, custom_dataloaders
    )
    if custom_dataloaders is not None:
        ...
        for batch in testloader:
            _preprocessed.append(batch[1])
        X = torch.cat(_preprocessed)
    # Use subsample of the data if specified and subsample is smaller than dataset
    if (
        init_subsample_size is not None
        and init_subsample_size > 0
        and init_subsample_size < X.shape[0]
    ):
        rand_idx = random_state.choice(X.shape[0], init_subsample_size, replace=False)
        subsampleloader = get_dataloader(
            X[rand_idx], batch_size=batch_size, shuffle=False, drop_last=False
        )
    else:
    ...
    input_centers, P, V, beta_weights = enrc_init(
        data=embedded_data,
        n_clusters=n_clusters,
        device=device,
        init=init,
    ...
    param_dict = [
        dict({"params": neural_network.parameters()}, **clustering_optimizer_params),
        dict({"params": [enrc_module.V]}, **clustering_optimizer_params),
        dict(
            {"params": [enrc_module.beta_weights]}, **clustering_optimizer_beta_params
        ),
    ]
    optimizer = optimizer_class(param_dict)
    ...
    cluster_labels_before_reclustering = enrc_module.predict_batchwise(
        model=neural_network, dataloader=testloader, device=device, use_P=True
    )
    # Recluster
    if final_reclustering:
        if debug:
            print("Recluster")
        enrc_module.recluster(
            dataloader=subsampleloader,
            model=neural_network,
            device=device,
            optimizer_params=clustering_optimizer_params,
            optimizer_class=optimizer_class,
        ...
        cluster_labels = enrc_module.predict_batchwise(
            model=neural_network, dataloader=testloader, device=device, use_P=True
        )
        if debug:
    ...
    cluster_centers = [
        centers_i.detach().cpu().numpy() for centers_i in enrc_module.centers
    ]
    ...
    return (
        ...
        n_clusters,
        neural_network,
        cluster_labels_before_reclustering,
    )
...
class ENRC(_AbstractDeepClusteringAlgo):
    ...
    def __init__(
        ...
        init_kwargs: dict = None,
        init_subsample_size: int = 10000,
        random_state: np.random.RandomState | int = None,
        custom_dataloaders: tuple = None,
        augmentation_invariance: bool = False,
        final_reclustering: bool = True,
        debug: bool = False,
    ):
        super().__init__(
            batch_size,
        ...
        if init in available_init_strategies():
            ...
            raise ValueError(
                f"init={init} does not exist, has to be one of {available_init_strategies()}."
            )
    ...
    def predict(
        ...
        if dataloader is None:
            dataloader = get_dataloader(
                X, batch_size=self.batch_size, shuffle=False, drop_last=False
            )

        self.neural_network.to(self.device)
        predicted_labels = enrc_predict_batchwise(
            V=torch.from_numpy(self.V).float().to(self.device),
            centers=[
    ...
    def transform_full_space(self, X: np.ndarray, embedded=False) -> np.ndarray:
        ...
        if not embedded:
            ...
            emb = encode_batchwise(
                dataloader=dataloader, neural_network=self.neural_network
            )
        else:
            emb = X
    ...
    def transform_subspace(
        self, X: np.ndarray, subspace_index: int = 0, embedded: bool = False
    ) -> np.ndarray:
        """
    ...
    def plot_subspace(
        ...
        if self.labels_ is None:
            raise Exception(
                "The ENRC algorithm has not run yet. Use the fit() function first."
            )
        if labels is None:
        ...
        plot_scatter_matrix(
            self.transform_subspace(X, subspace_index),
            labels,
    ...
    def reconstruct_subspace_centroids(self, subspace_index: int = 0) -> np.ndarray:
        ...
        centers_rec = self.neural_network.decode(
            torch.from_numpy(centers_rot_back).float().to(self.device)
        )
