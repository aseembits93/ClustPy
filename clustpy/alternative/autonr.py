...
import numpy as np
from clustpy.alternative.nrkmeans import (
    NrKmeans,
    _get_precision,
    _create_full_rotation_matrix,
)
...
def _autonr(
    ...
    """
    max_subspaces, max_n_clusters, max_distance, precision = _check_input_parameters(
        X, nrkmeans_repetitions, max_subspaces, max_n_clusters, max_distance, precision
    )
    all_mdl_costs = []
    # Default number of clusters
    n_clusters = [1]
    # Use first values as benchmark
    best_nrkmeans, best_mdl_overall, best_subspace_costs = _execute_nrkmeans(
        X,
        n_clusters,
        1,
        random_state,
    ...
    all_mdl_costs.append(
        _Nrkmeans_Mdl_Costs(True, best_mdl_overall, "noise_space_split")
    )
    ...
    while len(n_clusters) < max_subspaces:
        if debug:
            print("==================================================")
            print(
                "Start next iteration with: "
                + str(best_nrkmeans.n_clusters)
                + " / best costs: "
                + str(best_mdl_overall)
            )
            print("==================================================")
        ...
        for subspace_nr in order:
            if debug:
                print("==================================================")
                print(
                    "Try splitting subspace_nr {0} with n_clusters = [{1}] and m = {2}. Costs = {3}".format(
                        subspace_nr,
                        best_nrkmeans.n_clusters[subspace_nr],
                        best_nrkmeans.m[subspace_nr],
                        best_subspace_costs[subspace_nr],
                    )
                )
            ...
            if (
                best_nrkmeans.m[subspace_nr] == 1
                and best_nrkmeans.n_clusters[subspace_nr] > 1
            ):
                continue
            ...
            if (
                len(best_nrkmeans.n_clusters) > 1
                and mdl_threshold_split < best_subspace_costs[subspace_nr]
            ):
                # Get parameters for full space execution
                n_clusters_full, centers_full, P_full, V_full = (
                    _get_full_space_parameters_split(
                        X, best_nrkmeans, nrkmeans_split, subspace_nr
                    )
                )
                if debug:
                    print("==================================================")
                    print(
                        "Full space execution with n_clusters = {0}. Current best costs = {1}".format(
                            n_clusters_full, best_mdl_overall
                        )
                    )
                nrkmeans, mdl_cost, subspace_costs = _execute_nrkmeans(
                    X,
                    n_clusters_full,
                    1,
                    random_state,
                ...
                all_mdl_costs.append(
                    _Nrkmeans_Mdl_Costs(
                        ...
                        mdl_cost,
                        "noise_space_split"
                        if split_cluster_count
                        else "cluster_space_split",
                    )
                )
            ...
            # If number of subspaces was 1, check if total mdl split was smaller than best mdl overall
                ...
                len(best_nrkmeans.n_clusters) == 1
                and mdl_total_split < best_mdl_overall
            ):
                if debug:
                    print("!!! Better solution found !!!")
        ...
        if better_found_since_merging:
            if (
                not better_solution_found
                or len(best_nrkmeans.n_clusters) == max_subspaces
            ):
                (
                    best_nrkmeans,
                    best_subspace_costs,
                ...
                if (
                    len(best_nrkmeans.n_clusters) == max_subspaces
                    or not better_found_merge
                ):
                    break
        else:
            break
...
def _check_input_parameters(
    ...
    if X.shape[1] < 2:
        raise ValueError(
            "A minimum of 2 dimensions is needed. You input data has only 1 dimension."
        )
...
def _execute_nrkmeans(
    ...
    if debug:
        print("--------------------------------------------------")
        print(
            "[AutoNR] Next try with n_clusters = {0} ( {1} repetitions )".format(
                n_clusters, nrkmeans_repetitions
            )
        )
    # Prepare parameters
    ...
    if debug:
        print(
            "[AutoNR] Output n_clusters = {0} / m = {1}".format(
                best_nrkmeans.n_clusters, best_nrkmeans.m
            )
        )
...
def _split_noise_space(
    ...
    while n_clusters[0] <= max_n_clusters:
        ...
        if sum_subspace_costs < mdl_threshold_split:
            ...
            if (
                nrkmeans_split is not None
                and abs(subspace_costs[-1] - subspace_costs_split[-1])
                < abs(subspace_costs[-1]) * similarity_threshold
                and nrkmeans.m[-1] == nrkmeans_split.m[-1]
            ):
        ...
        if nrkmeans.have_subspaces_been_lost():
            ...
            all_cluster_scatter_matrices = np.zeros(
                (nrkmeans.n_clusters[0], X_subspace.shape[1], X_subspace.shape[1])
            )
            for cluster_id in range(nrkmeans.n_clusters[0]):
                centered_points = (
                    X_subspace[nrkmeans.labels_[:, 0] == cluster_id]
                    - nrkmeans.cluster_centers[0][cluster_id]
                )
                all_cluster_scatter_matrices[cluster_id] = np.matmul(
                    centered_points.T, centered_points
                )
            # Split cluster with largest variance
            if len(nrkmeans.n_clusters) == 2:
                centers = [
                    _split_largest_cluster(
                        nrkmeans.V,
                        nrkmeans.m[0],
                        nrkmeans.P[0],
...
def _split_cluster_space(
    ...
    while True:
        if n_clusters[0] * n_clusters[1] < split_cluster_count or 1 in n_clusters:
            if (
                single_change_index is not None
                or nrkmeans_split.have_subspaces_been_lost()
            ):
                break
            else:
                mdl_cost_1 = np.inf
                ...
                n_clusters[0] -= 1
                centers = [
                    _merge_nearest_centers(nrkmeans_split.cluster_centers[0]),
                    nrkmeans_split.cluster_centers[1],
                ]
                if (
                    n_clusters[0] * n_clusters[1] >= split_cluster_count
                    and 1 not in n_clusters
                ):
                    nrkmeans_1, mdl_cost_1, subspace_costs_1 = _execute_nrkmeans(
                        X_subspace,
                        n_clusters,
                ...
                n_clusters[1] -= 1
                centers = [
                    nrkmeans_split.cluster_centers[0],
                ...
                if (
                    ...
                    nrkmeans_2, mdl_cost_2, subspace_costs_2 = _execute_nrkmeans(
                        X_subspace,
                        n_clusters,
                        nrkmeans_repetitions if centers is None else 1,
                        random_state,
        ...
        if single_change_index is None:
            ...
            if centers is not None:
                centers = [
                    _merge_nearest_centers(nrkmeans.cluster_centers[0]),
            ...
            for i in range(len(best_nrkmeans.n_clusters) - 1):
                for j in range(i + 1, len(best_nrkmeans.n_clusters)):
                    ...
                    if debug:
                        ...
                        print(
                            "Try merging subspace_nr {0} with n_clusters = [{1}] and subspace_nr {2} with n_clusters = [{3}]. Combined costs = {4}".format(
                                i,
                                best_nrkmeans.n_clusters[i],
                                j,
                                best_nrkmeans.n_clusters[j],
                    ...
                    if len(best_nrkmeans.n_clusters) > 2:
                        X_subspace = np.c_[
                            best_nrkmeans.transform_subspace(X, i),
                            best_nrkmeans.transform_subspace(X, j),
                        ]
                    else:
                        X_subspace = X.copy()
                    ...
                    centers = []
                    turned_centers_1 = np.matmul(
                        best_nrkmeans.cluster_centers[i], best_nrkmeans.V
                    )[:, best_nrkmeans.P[i]]
                    turned_centers_2 = np.matmul(
                        best_nrkmeans.cluster_centers[j], best_nrkmeans.V
                    )[:, best_nrkmeans.P[j]]
                    ...
                    for n in reversed(
                        range(
                            max(
                                best_nrkmeans.n_clusters[i], best_nrkmeans.n_clusters[j]
                            ),
                            best_nrkmeans.n_clusters[i] * best_nrkmeans.n_clusters[j]
                            + 1,
                        )
                        ...
                        all_mdl_costs.append(
                            _Nrkmeans_Mdl_Costs(
                                len(best_nrkmeans.n_clusters) == 2,
                                best_mdl_overall
                                - best_subspace_costs[i]
                        ...
                        if sum_subspace_costs < mdl_threshold_merge:
                            ...
                            # Prepare values for next iteration
                            centers = [
                                _merge_nearest_centers(
                                    nrkmeans_merge.cluster_centers[0]
                                )
                            ]
                        else:
                            # pass
                    ...
                    if (
                        len(best_nrkmeans.n_clusters) > 2
                        and mdl_threshold_merge
                        < best_subspace_costs[i] + best_subspace_costs[j]
                    ):
                        # Get parameters for full space execution
                        n_clusters_full, centers_full, P_full, V_full = (
                            _get_full_space_parameters_merge(
                                X, best_nrkmeans, nrkmeans_merge, i, j
                            )
                        )
                        if debug:
                            print("==================================================")
                        ...
                        all_mdl_costs.append(
                            _Nrkmeans_Mdl_Costs(True, mdl_cost, "cluster_space_merge")
                        )
                        if mdl_cost < best_mdl_overall:
                            if debug:
                                print("!!! Better solution found !!!")
                    ...
                    # If number of subspaces was 2, check if total mdl merge was smaller than best mdl overall
                        ...
                        len(best_nrkmeans.n_clusters) <= 2
                        and mdl_total_merge < best_mdl_overall
                    ):
                        if debug:
                            print("!!! Better solution found !!!")
...
def _split_largest_cluster(
    ...
    n_points = [
        len(labels_subspace[labels_subspace == i]) for i in range(len(centers_subspace))
    ]
    # Index of the cluster with the largest variance
    cropped_V = V[:, P_subspace]
    turned_scatter = np.matmul(
        np.matmul(cropped_V.transpose(), scatter_matrices_subspace), cropped_V
    )
    variances = [
        np.trace(s) / n_points[i] / m_subspace for i, s in enumerate(turned_scatter)
    ]
    max_variances_id = np.argmax(variances)
    # For first center add variance, for second remove variance
    push_factor = (
        scatter_matrices_subspace[max_variances_id].diagonal()
        / n_points[max_variances_id]
        / m_subspace
    )
...
def _merge_nearest_centers(centers_subspace: np.ndarray) -> np.ndarray:
    ...
    # Construct the new centers array without the merged ones
...
def _find_two_closest_centers(centers_subspace: np.ndarray) -> (int, int):
    ...
    """
    # Compute pairwise distances using cdist to avoid large intermediate arrays
    distances = cdist(centers_subspace, centers_subspace, "euclidean")
    # Fill distances to themselves with infinity to ignore them
    np.fill_diagonal(distances, np.inf)
    # Find the two indices with the minimum distance
    index_1, index_2 = divmod(distances.argmin(), distances.shape[1])
    return index_1, index_2


def _get_full_space_parameters_split(
    X: np.ndarray, best_nrkmeans: NrKmeans, nrkmeans_split: NrKmeans, subspace: int
    ...
    centers_from_subspace = [
        [
            np.mean(X[nrkmeans_split.labels_[:, i] == j], axis=0)
            for j in range(nrkmeans_split.n_clusters[i])
        ]
        for i in range(nrkmeans_split.labels_.shape[1])
    ]
    centers_new += centers_from_subspace
    # Update the rotation matrix with the rotation from the splitted subspace_nr
    V_F = _create_full_rotation_matrix(
        best_nrkmeans.V.shape[0], best_nrkmeans.P[subspace], nrkmeans_split.V
    )
    ...
    P_from_subspace = [
        np.array([P_new[subspace][i] for i in p]) for p in nrkmeans_split.P
    ]
    del P_new[subspace]
    ...
    n_clusters_new, centers_new, P_new = _remove_multiple_noise_spaces(
        n_clusters_new, centers_new, P_new
    )
    # Return parameters for the full space execution
    return n_clusters_new, centers_new, P_new, V_new


def _get_full_space_parameters_merge(
    X: np.ndarray,
    best_nrkmeans: NrKmeans,
    nrkmeans_merge: NrKmeans,
    subspace_1: int,
    ...
    centers_from_subspace = [
        [
            np.mean(X[nrkmeans_merge.labels_[:, i] == j], axis=0)
            for j in range(nrkmeans_merge.n_clusters[i])
        ]
        for i in range(nrkmeans_merge.labels_.shape[1])
    ]
    centers_new += centers_from_subspace
    # Order the subspaces with the one with the most clusters first
...
def _remove_multiple_noise_spaces(
    n_clusters: list, centers: list, P: list
) -> (list, list, list):
    """
...
class _Nrkmeans_Mdl_Costs:
    ...
    def __init__(
        self, full_space_execution: bool, costs: float, originates_from_operation: str
    ):
        if originates_from_operation not in [
            "cluster_space_split",
            "noise_space_split",
            "cluster_space_merge",
        ]:
            raise ValueError(
                "Type must be cluster_space_split, noise_space_split or cluster_space_merge"
            )
...
class AutoNR(BaseEstimator, ClusterMixin):
    ...
    def __init__(
        ...
        max_distance: float = None,
        precision: float = None,
        similarity_threshold: float = 1e-5,
    ...
    def fit(self, X: np.ndarray, y: np.ndarray = None) -> "AutoNR":
        ...
        """
        nrkmeans, mdl_costs, all_mdl_costs = _autonr(
            X,
            self.nrkmeans_repetitions,
            self.outliers,
            self.max_subspaces,
            self.max_n_clusters,
    ...
    def plot_mdl_progress(self) -> None:
        ...
        if self.nrkmeans_ is None:
            raise Exception(
                "The AutoNR algorithm has not run yet. Use the fit() function first."
            )
        # Plot line with all costs
        mdl_costs = np.array(
            [nrkmeans_mdl.costs for nrkmeans_mdl in self.all_mdl_costs_]
        )
        fig, ax = plt.subplots()
        for i in range(len(mdl_costs) - 1):
            line_color = self.all_mdl_costs_[i + 1].get_line_color()
            ax.plot(
                (i, i + 1), (mdl_costs[i], mdl_costs[i + 1]), color=line_color, zorder=1
            )
        # Plot circle for each full execution (leave out best solution)
        full_executions = np.array(
            [
                i
                for i, nrkmeans_mdl in enumerate(self.all_mdl_costs_)
                if nrkmeans_mdl.full_space_execution
            ]
        )  # indices of full executions
        min_index_full_executions = np.argmin(
            mdl_costs[full_executions]
        )  # index of min value in full_executions array
        min_index_complete = full_executions[
            min_index_full_executions
        ]  # index of min value in complete dataset
        full_executions_wo_best = np.delete(full_executions, min_index_full_executions)
        plt.scatter(
            full_executions_wo_best,
            mdl_costs[full_executions_wo_best],
            alpha=0.7,
            color="blue",
            marker="o",
            zorder=2,
        ...
        # Add legend
        legend_elements = [
            Line2D([0], [0], color="brown", lw=2, label="Noise space split"),
            Line2D([0], [0], color="orange", lw=2, label="Cluster space split"),
            Line2D([0], [0], color="magenta", lw=2, label="Cluster space merge"),
            Line2D(
                [0], [0], marker="o", color="blue", markersize=7, label="Full space run"
            ),
            Line2D(
                [0], [0], marker="o", color="green", markersize=7, label="Best result"
            ),
        ]
        ax.legend(handles=legend_elements, loc="upper right")
        plt.show()

    def dissolve_noise_space(
        self, X: np.ndarray = None, random_feature_assignment: bool = True
    ) -> NrKmeans:
        """
